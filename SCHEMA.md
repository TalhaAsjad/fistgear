...
- First admin is bootstrapped manually: `await db.update(user).set({ role: "admin" }).where(eq(user.email, "..."))`
...
- Application uses upsert: `await db.insert(cartItem).values({ userId, variantId, quantity: 1 }).onConflictDoUpdate({ target: [cartItem.userId, cartItem.variantId], set: { quantity: sql`${cartItem.quantity} + 1` } })`
